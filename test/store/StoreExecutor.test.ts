...
describe("aitianyu-cn.node-module.tianyu-store.store.StoreExecutor", () => {
    ...
    const reducersMap: any = {
        increase: function (this: IActionDispatch, state: IState, _params: any): Promise<IState> {
            const newState = getNewState(state, ["time"], state.time + 1);
            return newState;
        },
        decrease: function (this: IActionDispatch, state: IState, _params: any): Promise<IState> {
            const newState = getNewState(state, ["time"], state.time - 1);
            return newState;
        },
        add: function (this: IActionDispatch, state: IState, params: any): Promise<IState> {
        ...
        sub: function (this: IActionDispatch, state: IState, params: any): Promise<IState> {
    ...
    it("test for get reducer failure case", (done) => {
        ...
        jest.spyOn(storeEntity, "postError").mockImplementation((actions: Action<any>[], error: string) => {
            ...
            expect(error).toEqual("error with unknown reason");
            done();
        });
    ...
    it("test for reducer execution failure case", (done) => {
        ...
        jest.spyOn(storeEntity, "postError").mockImplementation((actions: Action<any>[], error: string) => {
            expect(actions).toEqual(dispatcher.getAll());
            expect(error).toEqual("params sub value empty");
            done();
        });
